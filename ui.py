...
def set_shortcut(key, label, root, cards, config, save_config, play_sound):
    if label is None:
...
def build_ui(root, config, save_config, play_sound, rows=3, cols=4):
    total = rows * cols
    cards = {}
    ...
    for i in range(total):
        ...
        shortcut_btn = ttk.Button(shortcut_row, text="Set Shortcut", bootstyle="warning",
                                  command=partial(set_shortcut, key, cards.get(key, {}).get("shortcut_label", None),
                                                  root, cards, config, save_config, play_sound))
